...
# ////////////LED HOLE///////////////
LED_L = (f_u[0] - (f_r[0] - f_u[0]) - LED_HOLE_W / 2, f_lt[1])
LED_R = (f_y[0] + (f_y[0] - f_x[0]) - LED_HOLE_W / 2, f_lt[1])
...
def create_support(y_pos):
    ...
    # 上面的支撑需要开USB和开关
    if y_pos >= W / 2:
        v_usb = Part.makeBox(14.2 + CUT_COMM, 6.0 + CUT_COMM, T*2)
        v_usb.translate(App.Vector(s_length / 2 + 14.2/2, 3.0, -T/2))
        support = support.cut(v_usb)

        v_power = Part.makeCylinder(FUNC_D/2 + CUT_COMM, T*2)
        v_power.translate(App.Vector(s_length *0.8, FUNC_D/2+1.0, -T/2))
